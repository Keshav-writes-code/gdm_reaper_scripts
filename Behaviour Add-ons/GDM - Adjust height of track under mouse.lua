--[[
  @description adjust height of track under mouse
  @about use mouse wheel to adjust height of the mouse with this script. adjust the height of track under mouse
  @version 0.1
  @license MIT
  @author GDM
  @links
...
function main()
	...
	elseif string.find(context, "env") or string.find(context, "envelope") then --for envelopes
		...
		local active, visible, armed, inLane, laneHeight, defaultShape, _, _, _, _, faderScaling =
			reaper.BR_EnvGetProperties(br_env, false)

		if mouseScroll > 0 then
			if laneHeight > 500 then
				return
			end
